...
def replace_vars_in_logic(logic_raw, q_map):
    """BASE 로직 내의 변수를 재정렬된 Q번호로 치환 (MA/RK 대응)"""
...
def process_html_content(content):
    ...
    for i, q_div in enumerate(items):
        ...
        if prev_p:
            ...
            if p_cond:
                # 여기서 replace_vars_in_logic을 호출하여 Q48_1=1 형태로 바꿈
                base_str = f"BASE=({replace_vars_in_logic(p_cond, q_map)}) "
...
if check_password():
    ...
    if uploaded_file is not None:
        ...
        if content:
            ...
            st.error("올바른 ISAS5 HTML 파일이 아닙니다.")
